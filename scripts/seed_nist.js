...
async function seedNIST() {
    try {
        ...
        console.log("Conectado a MongoDB para seeding de NIST SP 800-53 r5...");

        // 1. Check if the framework already exists or create it
        ...
        if (framework) {
            ...
            // Clear existing requirements for this framework to avoid duplicates
        ...
        // 2. Map and insert the new requirements
        const reqsToSave = [];
        ...
        for (const r of nistControls) {
            if (!seenCodes.has(r.code)) {
                ...
                console.log(`Duplicado ignorado: ${r.code}`);
            }
        }
